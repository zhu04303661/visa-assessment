...
class GTVTaskEnvironment(TaskEnvironment):
    ...
    def evaluate(self, sample: Sample, generator_output) -> EnvironmentResult:
        ...
        try:
            # 解析生成器的回答
            if isinstance(generator_output.final_answer, str):
                try:
                    answer_data = json.loads(generator_output.final_answer)
                except json.JSONDecodeError:
                    answer_data = {"text": generator_output.final_answer}
            else:
                answer_data = generator_output.final_answer
